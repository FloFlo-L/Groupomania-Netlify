...
export default function Card({ post }) {
  ...
  function updatePost() {
    ...
    fetch(`https://flofloflogroupomaniaapp.herokuapp.com/api/post/${post._id}`, requestModify);
    window.location = '';
  }

  fetch(`https://flofloflogroupomaniaapp.herokuapp.com/api/post/${post._id}`)
    .then((res) => res.json())
    .then((res2) => {
  ...
  function deletePost() {
    ...
    fetch(`https://flofloflogroupomaniaapp.herokuapp.com/api/post/${post._id}`, requestDelete).then(
      (res) => {
        if (res.status === 200) {
          window.location = '/';
        }
